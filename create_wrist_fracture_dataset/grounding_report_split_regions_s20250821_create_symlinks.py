...
def setup_directories(output_dir):
    ...
    os.makedirs(os.path.join(output_dir, 'labels_raw'), exist_ok=True)
...
def copy_label_file(src, dst):
    """Copy a label file with robust error handling"""
    # Convert to absolute paths
    src_abs = Path(src).absolute()
    dst_abs = Path(dst).absolute()
    
    # Handle lab-share path
    src_str = str(src_abs).replace('/fileserver/', '/lab-share/')
    src_abs = Path(src_str)
    
    # Check if source file exists
    if not os.path.exists(src_abs):
        print(f"Warning: Label file does not exist: {src_abs}")
        return False
    ...
    try:
        ...
        print(f"Error copying label file from {src_abs} to {dst_abs}: {e}")
        return False
...
def process_reports(input_csv_path, image_dir, label_dir, output_dir):
    ...
    output_labels_dir = os.path.join(output_dir, 'labels_raw')
        ...
        for png_file in png_files:
            ...
            if expected_fractures == 0:
                ...
                if actual_fractures == expected_fractures:
                    # Fracture counts match - create symlink for image, copy label file
                    print(f"    Fracture counts match - creating symlink for image, copying label file")
                    
                    # Create symlink for image
                    if create_symlink(png_file, output_png):
                        successful_symlinks += 1
                    
                    # Copy label file (not symlink)
                    if copy_label_file(label_file, output_label):
                        successful_copies += 1
                        print(f"    Copied label file: {output_label}")
    ...
    print(f"Successful image symlinks created: {successful_symlinks}")
    print(f"Successful label files copied: {successful_copies}")
