...
def process_scan_id_directory(args):
    scan_id_path, scan_id, overwrite, no_report_log, no_png_log, process_id, progress_file = args
    ...
    # Only extract report if it doesn't exist or is empty
    if not os.path.exists(report_txt_path) or os.path.getsize(report_txt_path) == 0:
        for root, dirs, files in os.walk(scan_id_path):
...
def main(main_dir, overwrite=False, num_processes=None, reverse_order=False):
    ...
    for i, scan_id in enumerate(items):
        ...
        process_args.append((scan_id_path, scan_id, overwrite, no_report_log, no_png_log, i, progress_file))
