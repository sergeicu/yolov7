...
def copy_png_files(source_dir, dest_dir, logger):
    """Copy PNG files to the destination directory using os.walk, skipping existing ones."""
    ...
    logger.info(f"Starting PNG file extraction from: {source_dir}")
    ...
    for root, dirs, files in os.walk(source_dir):
        ...
        for png_file in png_files:
            source_file = os.path.join(root, png_file)
            dest_file = os.path.join(dest_dir, png_file)
    ...
    logger.info(f"Folders with PNG files: {total_folders_with_files}")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Copy all PNG files to a common directory using os.walk, skipping existing ones.")
    ...
    parser.add_argument("--dest_dir", 
                       default='/lab-share/Rad-Afacan-e2/Public/serge/code/llm/experiments/yolov7/create_wrist_fracture_dataset/pngs_aug2025/',
                       help="Directory to copy all PNG files to.")
    parser.add_argument("--log_file", 
                       default=None,
                       help="Path to log file (default: logs/png_extraction_YYYYMMDD_HHMMSS.log)")
    ...
    if args.log_file is None:
        ...
        args.log_file = f"logs/png_extraction_{timestamp}.log"
    
    # Setup logging
    logger = setup_logging(args.log_file)
    
    # Start the extraction process
    copy_png_files(args.source_dir, args.dest_dir, logger)
