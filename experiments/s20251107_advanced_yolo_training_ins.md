# YOLOv7 Class Imbalance Solutions for Pediatric Fracture Detection

**Date**: 2025-11-10
**Project**: Fine-tuning YOLOv7 for bone-specific fracture detection
**Dataset**: ~100k pediatric fracture images with severe class imbalance (4% scaphoid, 50%+ distal radius)

---

## Executive Summary & Critical Decision

### THE KEY QUESTION: Should we relabel to 4 bone-specific classes or keep generic "fracture" class?

**RECOMMENDATION: Option B - Relabel to 4 Bone-Specific Classes** ✅

**Why this is critical:**
- Your current model: Detects generic "fracture" as class 3
- Your data: 4 bone types (distal_radius, distal_ulna, scaphoid, ulna_styloid) with 12:1 imbalance
- **The problem**: If all fractures stay as class 3, focal loss cannot apply class-specific weights

**Impact comparison:**
```
Option A (Keep class 3):
- ✅ Balanced batch sampling: YES
- ❌ Class-specific focal loss weights: NO (all are class 3!)
- ❌ Per-class performance tracking: NO
- Result: Limited effectiveness for 4% minority class

Option B (Relabel to 4 classes):
- ✅ Balanced batch sampling: YES
- ✅ Class-specific focal loss weights: YES (6.25x weight for scaphoid!)
- ✅ Per-class performance tracking: YES
- ✅ Backbone transfers perfectly (layers 0-50 unchanged)
- ⚠️ Detection head reinitialized (relearns in 5-10 epochs)
- Result: Full power of both techniques
```
...
# New class structure:
nc: 4
names: ['distal_radius', 'distal_ulna', 'scaphoid', 'ulna_styloid']

# Focal loss weights (inverse frequency, normalized):
fl_alpha: [0.08, 0.13, 1.0, 0.25]  # Scaphoid gets 12.5x more weight than distal_radius
```

**Implementation effort:**
- Relabeling dataset: 1-2 days (you have bone metadata from bone detection YOLO)
- Code modifications: 2-3 hours (focal loss + balanced sampling)
- Training: 1 week (progressive fine-tuning)
- **Total**: ~2 weeks vs 6+ months of suboptimal results with Option A

---

## Three-Pronged Attack Strategy

To address the extreme class imbalance (4% scaphoid), we implement **three complementary techniques**:

### 1. Class-Balanced Focal Loss ⚖️
**Purpose**: Reweight loss to prioritize rare classes
**Implementation**: Modify focal loss with per-class α weights
```python
fl_gamma: 2.0  # Focus on hard examples
fl_alpha: [0.08, 0.13, 1.0, 0.25]  # Scaphoid weighted 12.5x higher
```

### 2. Balanced Batch Sampling 🎲
**Purpose**: Ensure every batch contains rare class examples
**Implementation**: Custom sampler with diversity preservation
```python
# Each batch guaranteed to have 2+ scaphoid examples
# Scaphoid data repeated ~3x per epoch with strong augmentation
```

### 3. Progressive Fine-Tuning 🔄
**Purpose**: Prevent catastrophic forgetting of pretrained features
**Implementation**: 4-phase gradual unfreezing with knowledge distillation
```python
Phase 1: Freeze backbone (0-50), train head only (15 epochs)
Phase 2: Unfreeze upper backbone (38-105), small LR (20 epochs)
Phase 3: Unfreeze mid backbone (25-105), regularization (20 epochs)
Phase 4: Full fine-tuning with strong constraints (15 epochs)
```
...
### Week 1: Dataset Relabeling & Setup
**Tasks:**
1. Create relabeling script using bone detection metadata
2. Update class IDs: 3 → {0,1,2,3} based on bone type
3. Create new data YAML with nc=4
4. Verify relabeling accuracy (spot check 100 random images)

**Script template:**
```python
# relabel_fractures.py
import os
from pathlib import Path

# Load bone metadata (from bone detection YOLO or manual labels)
bone_metadata = load_metadata()  # image → bone_type mapping
...
for img_path, bone_type in bone_metadata.items():
    ...
    for line in lines:
        ...
        if parts[0] == '3':  # If it's a fracture (old class ID)
            parts[0] = str(class_map[bone_type])
...
### Week 2: Implement Focal Loss & Balanced Sampling
**Tasks:**
1. Modify `utils/loss.py` to support per-class focal loss weights
2. Implement `BalancedBatchSampler` in `utils/datasets.py`
3. Create hyperparameter file with class weights
4. Unit test both components

**Expected outcome:** Code ready for training

### Week 3-4: Progressive Fine-Tuning (Phase 1 & 2)
**Tasks:**
1. Phase 1: Freeze backbone, adapt detection head (15 epochs)
2. Phase 2: Unfreeze upper layers, gradual adaptation (20 epochs)
3. Monitor per-class mAP, especially scaphoid
4. Adjust hyperparameters if needed

**Success criteria:** Scaphoid mAP > 0.55 (baseline ~0.35)

### Week 5-6: Progressive Fine-Tuning (Phase 3 & 4) & Validation
**Tasks:**
1. Phase 3 & 4: Full fine-tuning with regularization (35 epochs)
2. Cross-validation on holdout test set
3. Evaluate on original dataset to check forgetting
4. Final model selection and documentation

**Success criteria:**
- Scaphoid mAP > 0.65
- Distal radius mAP maintained > 0.75
- Overall mAP > 0.70
...
The following sections contain comprehensive implementation details from specialized planning agents:
