...
def train(hyp, opt, device, tb_writer=None):
    ...
    # Get paths from data dictionary
    ...
    if pretrained:
        ...
        if isinstance(ckpt, dict) and 'model' in ckpt:
            ...
            if hasattr(model_to_inspect, 'model'):
                for i, m in enumerate(model_to_inspect.model):
                    print(f"Layer {i}: {m.__class__.__name__}")
        
        print(f"\n=== Loading ONLY MODEL WEIGHTS from {weights} ===")
    ...
    # Initialize loss function AFTER model is created
    compute_loss = ComputeLoss(model)  # Use standard loss

    # Calculate grid size first
    gs = max(int(model.stride.max()), 32)  # grid size (max stride)
    
    # Then verify image sizes
    ...
    # Initialize everything fresh from hyperparameters
    ...
    for k, v in model.named_modules():
        ...
        # Add all implicit parameter handling from og file
        if hasattr(v, 'im'):
    ...
    if not opt.notest or opt.noautoanchor:
        ...
        if hasattr(opt, 'additional_val') and opt.additional_val:
